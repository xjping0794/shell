...
# for example : export_load_3.sh -t1 bass15.bass1_02004_20140131 -t2 bass15.bass1_02004_20140131 -s tbs_test -p 2 
...
if [ $type == '1' ]; then
    ...
    ExportDbName="WEBDB209"
    ExportDbUser="biqt"
    ExportDbPassword="biqt123"
    LoadDbName="YNBIDB"
    LoadDbUser="aibass"
    LoadDbPassword="newbi1001"
    echo "---${export_tb1} start to export from bass2node to HISDB---and tablename of report is ${load_tb1}------\n"
...
Export()
{ 
	echo "判断目录是否存在，若不存在，则创建........\n"
	if [ -d /biyjetl/aibassyj/user/xiaojp/exportload ]; then
	print "/biyjetl/aibassyj/user/xiaojp/exportload is already exist...."
	else  
	mkdir /biyjetl/aibassyj/user/xiaojp/exportload 
	fi
	db2 "connect to ${ExportDbName} user ${ExportDbUser} using ${ExportDbPassword}"    
	if [ $? -ne 0 ]
	then 
	  printf "connect to ${ExportDbName} fail......\n"
	  return 1
  fi
  echo "${ExportDbName} connect success ........\n" 
  printf "export to /biyjetl/aibassyj/user/xiaojp/exportload/${export_tb1}.txt of del modified by COLDEL0x01 nochardel decplusblank striplzeros select * from  ${export_tb1}"
  ## modified by COLDEL0x01 nochardel decplusblank striplzeros
	db2 "export to /biyjetl/aibassyj/user/xiaojp/exportload/${export_tb1}.txt of del select * from ${export_tb1}"
	if [ $? -ne 0 ]
	then 
	  printf "export table ${export_tb1} fail......"
	  return 1
  fi
  db2 terminate
}	
...
file_deal()
{
	...
	echo "cat ${source_table}.sql|sed -e "s/\"$soure_space\"/\"$tbs_space\"/g" -e "s/$source_schema/$target_schema/g" -e "s/$source_table/$target_table/g" -e "s/CONNECT TO ${ExportDbName} USER ${ExportDbUser}/CONNECT TO ${LoadDbName} USER ${LoadDbUser} using ${LoadDbPassword}/g" "
...
Load()
{
  ...
  echo " ${LoadDbName} conect success.....\n"
  ##modified by COLDEL0x01 nochardel decplusblank
  db2 "load client FROM /biyjetl/aibassyj/user/xiaojp/exportload/${export_tb1}.txt of del  replace into ${load_tb1}"
  if [ $? -ne 0 ]
     then
        printf "Load table name :${load_tb1} fail."
        return 1
  fi
     db2 terminate
}
echo "strat export ${export_tb1}......"
Export
if [ $? -ne 0 ] 
   then 
      printf "export table ${export_tb1} fail......\n"
	    ...
	    printf "export table ${export_tb1} success......\n"
	    echo "get structure of ${export_tb1}......\n" 
      Get_Tbstructure
      echo "start Creating ${load_tb1}......\n" 
      Create
      if [ $? -ne 0 ] 
        then
          printf "create table name :${load_tb1} fail.\n"
          ...
          printf "create table name :${load_tb1} success...\n" 
          echo "start loading ${load_tb1}......\n" 
          Load
          if [ $? -ne 0 ]
            then
                printf "Load table name :${load_tb1} fail.\n"
                ...
                printf "Load table name :${load_tb1} success.\n"
          fi
      fi
fi
